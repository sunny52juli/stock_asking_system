...
3. 确认质量良好，**等待用户询问是否保存**,不要自动调用 save_screening_script
...
6. **等待用户决定** - 筛选成功后，等待用户选择是否保存脚本，不要自动保存
7. **⚠️ 参数命名规范（非常重要）**:
...
8. **🔗 变量命名一致性（至关重要）**:
   - ✅ **正确示例**: 
     ...
     {{
       "tools": [
         {{"var": "daily_return", "tool": "pct_change", "params": {{"column": "close"}}}},
         {{"var": "avg_volume_5d", "tool": "rolling_mean", "params": {{"column": "vol", "window": 5}}}}
       ],
       "expression": "(daily_return > 0.03) & (vol / avg_volume_5d > 1.5)"
     }}
     ```
   - ❌ **错误示例**: 
     ```json
     {{
       ...
       "expression": "(日涨幅 > 0.03)"  // ❌ 使用了中文，而非工具定义的 daily_return
     }}
     ```
   - **核心规则**: `expression` 和 `confidence_formula` 中引用的变量名必须与 `tools` 中定义的 `var` **完全一致**
   - 可以使用基础字段名：`close`, `open`, `high`, `low`, `vol`, `amount` 等
   - ❌ **不要使用中文字段名** (如 `日涨幅`, `收盘价`)，除非它直接对应数据列名
9. **📊 字段命名规范**:
