...
> 提供标准化的金融数据访问接口，基于 **Polars** 实现高性能数据处理。

## 📖 概述

DataHub 是系统的统一数据访问层，采用 **Repository Pattern**（仓储模式）设计，屏蔽底层数据源差异，为上层应用提供一致的数据访问接口。**v2.0 版本全面采用 Polars DataFrame**，实现零转换开销的高性能数据流。
...
- ✅ **自动切换**：本地/远程数据源智能切换
- ✅ **Polars 原生**：全程使用 Polars DataFrame，零 pandas 转换
...
| 特性 | Pandas | Polars | 优势 |
|------|--------|--------|------|
| 后端 | Python | Rust | 10-100x 性能提升 |
| 内存管理 | 拷贝为主 | 零拷贝 | 60% 内存节省 |
| 并行计算 | 单线程 | 自动多线程 | 充分利用多核 |
| 惰性求值 | ❌ | ✅ | 查询优化 |
| 类型系统 | 弱类型 | 强类型 | 更少错误 |

### DataHub 返回格式

**所有 DataHub API 现在返回 Polars DataFrame**：
...
df = loader.load_market_data(
    trade_date_for_pool="20240301",
    start_date="20240101",
    end_date="20240301"
)

# ✅ df 是 Polars DataFrame
...
# ✅ 返回 Polars DataFrame
data = repo.load(query)
type(data)  # polars.DataFrame
```

### 迁移注意事项

如果你之前使用 pandas API，需要调整为 Polars：

**Pandas → Polars 对照表**：

| Pandas | Polars | 说明 |
|--------|--------|------|
| `df[df["col"] > 10]` | `df.filter(pl.col("col") > 10)` | 过滤 |
| `df.groupby("col").agg({"val": "mean"})` | `df.group_by("col").agg(pl.col("val").mean())` | 分组聚合 |
| `df["col"].isin([1,2,3])` | `pl.col("col").is_in([1,2,3])` | 包含判断 |
| `df.dropna()` | `df.drop_nulls()` | 删除空值 |
| `df.copy()` | `df.clone()` | 复制 |
| `df.empty` | `df.is_empty()` | 判空 |
| `df["col"].nunique()` | `df["col"].n_unique()` | 唯一值计数 |
...
# 之前 (pandas)
# df_pd = df[df["close"] > 10]
# result = df_pd.groupby("ts_code").agg({"close": "mean"})

# 现在 (polars)
df_filtered = df.filter(pl.col("close") > 10)
result = df_filtered.group_by("ts_code").agg(pl.col("close").mean())
```
