...
ScreenerOrchestrator (编排器)
    │
    ├─ DataLoader          → 加载原始市场数据 (Polars DataFrame)
    │
    ...
    │       └─ 数据完整性检查
    │
    ├─ ComponentInitializer → 初始化 Agent 组件
...
def execute_screening(
    ...
    Args:
        screening_logic: {
            ...
            "tools": [
                {"name": "rolling_mean", "params": {...}, "output": "vol_ma20"}
            ],
            "confidence": "rank_normalize(vol / vol_ma20) * 0.6"
        }
        top_n: 返回前 N 只股票
