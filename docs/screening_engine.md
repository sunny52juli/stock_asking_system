# Screening Engine - 股票筛选引擎

> 高效执行股票筛选逻辑，支持动态指标计算和多条件过滤。

## 📖 概述

Screening Engine 是系统的核心执行引擎，负责将 Agent 生成的筛选逻辑转化为实际的股票筛选操作。采用向量化批量计算技术，能够在秒级时间内完成数千只股票的复杂指标计算和筛选。

### 核心特性

- ⚡ **高性能**：向量化批量计算，秒级处理数千只股票
- 🎯 **智能预筛选**：基于股票池规则快速过滤无效股票
- 🔧 **动态指标**：支持任意技术指标和自定义因子计算
- 📊 **多维排序**：支持截面归一化后的等权综合得分排序
- 💾 **脚本生成**：自动生成可复用的 Python 筛选脚本
- 🏭 **行业匹配**：模糊匹配引擎，支持自然语言行业查询

## 🏗️ 架构设计

### 数据流程（Polars 统一流）

```
┌─────────────────────────────────────┐
│   初始化阶段                         │
│  • datahub/loaders: 加载原始市场数据 │
│    - 返回 Polars DataFrame          │
│  • StockPoolService: 执行股票池过滤  │
│    - ST/停牌/上市天数过滤            │
│    - 行业过滤（LLM智能匹配）         │
│    - 价格/成交量/市值过滤            │
│    - 数据完整性过滤                  │
│    - ✅ 保持 Polars 格式             │
└──────────────┬──────────────────────┘
               │ (Polars DataFrame)
┌──────────────▼──────────────────────┐
│   Agent 执行阶段                     │
│  • Screening Logic (from Agent)     │
│    - expression: 筛选表达式          │
│    - tools: 指标计算工具列表         │
│    - confidence: 置信度公式          │
└──────────────┬──────────────────────┘
               │
┌──────────────▼──────────────────────┐
│     BatchCalculator                  │
│  • Polars 向量化计算技术指标         │
│  • 并行处理多只股票                   │
│  • 缓存中间结果                       │
└──────────────┬──────────────────────┘
               │
┌──────────────▼──────────────────────┐
│     FilterEngine                     │
│  • 执行布尔表达式                    │
│  • 计算置信度                        │
│  • Top-N 排序                        │
└──────────────┬──────────────────────┘
               │
┌──────────────▼──────────────────────┐
│     ScriptSaver                      │
│  • 生成 Python 脚本                  │
│  • 保存到 screening_scripts/         │
└─────────────────────────────────────┘
```

**关键优化**：
- **一次性过滤**：股票池过滤在主函数中执行一次，多个策略共享过滤后的数据
- **职责分离**：`StockPoolService` 独立处理过滤逻辑，`Orchestrator` 只负责流程编排

## 🏛️ 模块架构

### 组件关系图

```
ScreenerOrchestrator (编排器)
    │
    ├─ ComponentManager      → 管理所有组件初始化
    │   └─ create_bridge_tools(data_fn, stock_codes)
    │
    ├─ StockPoolService    → 股票池过滤服务
    │   └─ StockPoolFilter
    │       ├─ ST/停牌过滤
    │       ├─ 上市天数过滤
    │       ├─ 行业匹配 (IndustryMatcher)
    │       ├─ 价格/市值过滤
    │       └─ 数据完整性检查
    │
    └─ QueryExecutor       → 执行用户查询
        └─ ScreeningExecutor (筛选执行器)
            ├─ BatchCalculator      → 批量计算技术指标
            ├─ FilterEngine         → 表达式过滤
            └─ ScriptSaver          → 生成 Python 脚本
```

### 核心类说明

#### 1. ScreeningExecutor

**文件**：`src/screening/executor.py`

**职责**：筛选执行的主入口，协调整个筛选流程

**主要方法**：
```python
def run_screening(
    self,
    screening_logic: dict,
    top_n: int = 10
) -> list[dict]:
    """
    Args:
        screening_logic: {
            "expression": "(vol > vol_ma20 * 1.5) & (pct_1d > 0.03)",
            "tools": [
                {"var": "vol_ma20", "tool": "rolling_mean", "params": {"column": "vol", "window": 20}}
            ],
            "confidence_formula": "vol / vol_ma20 + pct_1d"
        }
        top_n: 返回前 N 只股票（默认 10）
    
    Returns:
        List of stock dicts with metrics and confidence scores.
    """
```

**排序逻辑更新**：
系统已移除旧的 `confidence` 字段依赖。现在通过解析 `confidence_formula` 中的变量，对候选股票进行**截面 Min-Max 归一化**后，按公式中涉及的指标进行**等权求和**（权重总和为 1），从而实现公平的多维度综合排序。

#### 2. StockPoolFilter

**文件**：`src/screening/stock_pool_filter.py`

**职责**：执行股票池过滤规则

**过滤顺序**：
1. ST 股票排除
2. 停牌股票排除
3. 上市天数检查
4. 行业关键词匹配（LLM 模糊匹配）
5. 价格区间过滤
6. 成交量/成交额过滤
7. 市值区间过滤
8. 数据完整性检查

**性能优化**：
- ✅ Polars 向量化过滤
- ✅ 并行处理多条件
- ✅ 早期退出（短路求值）

#### 3. BatchCalculator

**文件**：`src/screening/batch_calculator.py`

**职责**：批量计算技术指标和自定义因子

**支持指标**：
- 移动平均线 (MA, EMA)
- 相对强弱指数 (RSI)
- MACD
- 布林带 (Bollinger Bands)
- 成交量均线
- 涨跌幅
- 自定义滚动窗口统计

**🔀 工具依赖拓扑排序**：

BatchCalculator 会自动分析工具之间的依赖关系，并进行拓扑排序，确保依赖的工具先执行。

**依赖检测规则**：
- **基础数据列**：`close`, `open`, `high`, `low`, `volume`, `pct_chg` 等 → 无依赖
- **工具生成变量**：如果参数值（如 `column`）匹配其他工具的 `var` 名 → 存在依赖

**示例**：
```python
# Agent 生成的工具列表（可能顺序不正确）
tools = [
    {"var": "volatility_ma60", "tool": "rolling_mean", "params": {"column": "volatility_20d", "window": 60}},
    {"var": "volatility_20d", "tool": "volatility", "params": {"column": "close", "window": 20}},
]

# 系统自动排序后
# 1. volatility_20d (无依赖，先执行)
# 2. volatility_ma60 (依赖 volatility_20d，后执行)
```

**算法实现**：
- 使用 **Kahn's 算法**进行拓扑排序
- 构建依赖图：检查每个工具的参数是否引用其他工具的输出变量
- 检测循环依赖：如果发现循环依赖，发出警告并尝试恢复
- 时间复杂度：O(V + E)，V 为工具数量，E 为依赖关系数量

**日志输出**：
```
🔀 工具执行顺序调整：
   原始：volatility_ma60 → volatility_20d
   排序：volatility_20d → volatility_ma60
```

**最佳实践**：
虽然系统会自动排序，但建议在定义工具时按依赖层次组织：
1. **基础指标**（无依赖）：直接从原始数据计算
2. **派生指标**（依赖基础指标）：基于步骤1的结果
3. **标准化指标**（依赖前两步）：排名、归一化等

**计算策略**：
```python
# 按股票分组，并行计算
for ts_code in stock_codes:
    stock_data = df.filter(pl.col("ts_code") == ts_code)
    # 计算所有指标
    result = calculate_indicators(stock_data, tools)
```

#### 4. IndustryMatcher

**文件**：`src/screening/industry_matcher.py`

**职责**：行业关键词模糊匹配

**匹配算法**：
1. 提取用户查询中的行业关键词
2. 与标准行业列表进行相似度计算
3. 返回 Top-K 匹配行业

**示例**：
```python
matcher = IndustryMatcher()
matched = matcher.match("半导体芯片", top_k=3)
# 返回: ["半导体", "集成电路", "电子元件"]
```

#### 5. ScriptSaver

**文件**：`src/screening/script_saver.py`

**职责**：生成可复用的 Python 筛选脚本

**生成内容**：
- 完整的筛选逻辑代码
- 指标计算公式
- 结果保存逻辑
- 可视化代码（可选）

**保存路径**：
```
screening_scripts/
├── 放量突破/
│   ├── 放量突破_20260413_235007.py
│   └── 放量突破_20260414_002658.py
└── 跑赢大盘/
    └── 跑赢大盘_20260416_220131.py
```

## 📁 目录结构

```
src/screening/
├── __init__.py                     # 模块入口
├── batch_calculator.py            # 批量计算器 BatchCalculator
├── executor.py                    # 筛选执行器 ScreeningExecutor
├── expression_evaluator.py        # 表达式评估器 ExpressionEvaluator
├── industry_matcher.py            # 行业匹配器 IndustryMatcher
├── mcp_tool_runner.py             # MCP工具运行器 MCPToolRunner
├── namespace_builder.py           # 命名空间构建器 NamespaceBuilder
├── result_display.py              # 结果显示工具 ResultDisplayer
├── script_saver.py                # 脚本保存器 ScriptSaver
└── stock_pool_filter.py           # 股票池筛选器 StockPoolFilter
```

## 🔧 核心组件

### 1. ScreeningExecutor（筛选执行器）

主入口类，协调整个筛选流程。

**使用示例**：

```python
from src.screening import create_screening_executor
import polars as pl

# 准备数据
data = pl.DataFrame({...})  # 股票数据
index_data = pl.DataFrame({...})  # 指数数据（可选）

# 创建执行器
executor = create_screening_executor(
    data=data,
    screening_date="20260510",
    index_data=index_data
)

# 执行筛选
results = executor.run_screening(
    screening_logic={
        "expression": "(vol > vol_ma20 * 1.5) & (pct_1d > 0.03)",
        "tools": [
            {
                "tool": "rolling_mean",
                "params": {"column": "vol", "window": 20},
                "var": "vol_ma20"
            },
            {
                "tool": "pct_change",
                "params": {"column": "close", "periods": 1},
                "var": "pct_1d"
            }
        ],
        "confidence_formula": "rank_normalize(vol / vol_ma20) * 0.6 + rank_normalize(pct_1d) * 0.4"
    },
    top_n=20
)

print(f"筛选出 {len(results)} 只股票")
for stock in results[:5]:
    print(f"  {stock['ts_code']} {stock['name']} - 得分: {stock.get('confidence', 0):.2f}")
```

### 2. StockPoolFilter（股票池筛选器）

**文件**：`src/screening/stock_pool_filter.py`

**职责**：执行股票池过滤规则

**过滤顺序**：
1. ST 股票排除
2. 停牌股票排除
3. 上市天数检查
4. 行业关键词匹配（LLM 模糊匹配）
5. 价格区间过滤
6. 成交量/成交额过滤
7. 市值区间过滤
8. 数据完整性检查

**性能优化**：
- ✅ Polars 向量化过滤
- ✅ 并行处理多条件
- ✅ 早期退出（短路求值）

### 3. BatchCalculator（批量计算器）

**文件**：`src/screening/batch_calculator.py`

**职责**：通过 MCP 工具运行器和命名空间构建器，完成技术指标的向量化批量计算。

**支持的指标类型**：

| 指标类型 | 函数名 | 示例 |
|---------|--------|------|
| 移动平均 | `rolling_mean` | `rolling_mean({"column": "close", "window": 5})` |
| 最大值 | `rolling_max` | `rolling_max({"column": "high", "window": 20})` |
| 最小值 | `rolling_min` | `rolling_min({"column": "low", "window": 20})` |
| 涨跌幅 | `pct_change` | `pct_change({"column": "close", "periods": 1})` |
| RSI | `rsi` | `rsi({"column": "close", "window": 14})` |
| MACD | `macd` | `macd({"column": "close"})` |

### 4. ExpressionEvaluator（表达式评估器）

**文件**：`src/screening/expression_evaluator.py`

**职责**：负责向量化评估筛选表达式，统计匹配结果。

### 5. ScriptSaver（脚本保存器）

自动生成可复用的 Python 筛选脚本。

**使用示例**:

```python
from src.screening import ScriptSaver
from pathlib import Path

saver = ScriptSaver(output_dir=Path("app/screening_scripts"))

script_path = saver.save_script(
    strategy_name="放量突破",
    screening_logic=screening_logic,
    screening_date="20260413"
)

print(f"脚本已保存: {script_path}")
```

## 💡 使用示例

### 示例 1：基础筛选

```python
screening_logic = {
    "expression": "(close > ma5) & (ma5 > ma10) & (ma10 > ma20)",
    "tools": [
        {"tool": "rolling_mean", "params": {"column": "close", "window": 5}, "var": "ma5"},
        {"tool": "rolling_mean", "params": {"column": "close", "window": 10}, "var": "ma10"},
        {"tool": "rolling_mean", "params": {"column": "close", "window": 20}, "var": "ma20"}
    ],
    "confidence_formula": "rank_normalize(close / ma20)"
}

results = executor.run_screening(
    screening_logic=screening_logic,
    top_n=20
)
```

### 示例 2：复杂筛选（放量突破）

```python
screening_logic = {
    "expression": "(vol > vol_ma20 * 1.5) & (pct_1d > 0.03) & (close >= high_20 * 0.95)",
    "tools": [
        {"tool": "rolling_mean", "params": {"column": "vol", "window": 20}, "var": "vol_ma20"},
        {"tool": "pct_change", "params": {"column": "close", "periods": 1}, "var": "pct_1d"},
        {"tool": "rolling_max", "params": {"column": "high", "window": 20}, "var": "high_20"}
    ],
    "confidence_formula": "rank_normalize(vol / vol_ma20) * 0.4 + rank_normalize(pct_1d) * 0.3"
}

results = executor.run_screening(screening_logic=screening_logic, top_n=20)
```

## ⚙️ 配置说明

### 股票池配置 (`app/setting/stock_pool.yaml`)

```yaml
stock_pool:
  min_list_days: 180           # 最小上市天数
  exclude_st: true             # 排除ST股票
  exclude_suspended: true      # 排除停牌股票
  industry: null               # 行业过滤列表，null表示全市场
  
  # 数据质量参数
  min_completeness_ratio: 0.8  # 最小数据完整度 (0.0-1.0)
  max_missing_days: 5          # 最大缺失天数
  min_price: 2                 # 最低价格过滤
  max_price: 1000              # 最高价格过滤
  min_vol: 0                   # 最小成交量过滤，0表示不过滤
  min_amount: 100000           # 最小成交额过滤（单位千元）
  min_turnover: 0              # 最小换手率过滤，0表示不过滤
  min_total_mv: 1e+5           # 最小总市值过滤（单位万元）
  max_total_mv: 1e+8           # 最大总市值过滤（单位万元）
```

### 筛选引擎配置

```yaml
screening:
  max_candidates: 100         # 最大候选股票数
  default_top_n: 20           # 默认返回Top-N数量
  enable_script_save: true    # 是否启用脚本保存
  script_output_dir: "app/screening_scripts"  # 脚本输出目录
```

## 🧪 测试

```bash
pytest tests/src/screening/ -v
```

## 🔗 相关文档

- [主 README](../README.md)
- [DataHub](./datahub.md)
- [Agent System](./agent_system.md)
- [Backtest Engine](./backtest_engine.md)
