...
**[WARN] 关键规则：表达式只能使用变量名，不能直接调用工具函数！**
...
**[ERROR] 错误的表达式示例（严禁使用）**:
- `(close > rolling_mean(close, 20))` - [ERROR] **禁止在表达式中调用工具函数**
- `(vol > rolling_mean(vol, 5) * 1.5)` - [ERROR] **禁止在表达式中调用工具函数**
- `(close > high_20)` - [ERROR] **永远为False**：收盘价不可能高于包含当日的N日最高价
- `(vol < 0)` - 成交量不可能为负
- 同时要求多个极端条件导致结果为空

**💡 正确用法对比**:
- [OK] 突破均线：先在 tools 中定义变量（var: ma20, tool: rolling_mean），然后表达式用 `(close > ma20)`
- [ERROR] 错误逻辑：`(close > rolling_mean(close, 20))` - 直接在表达式中调用工具

**📖 完整规范**: 详见 `.stock_asking/rules/expression-design.md`

### 5. 表达式语法
- [OK] 正确：`(macd_result > 0) & (rsi_value < 30)` - 直接使用变量名
- [ERROR] 错误：`macd_result.macd > 0` - 禁止使用属性访问
- [OK] 正确：`(price_change > 0.03) & (vol_ma5 > vol_ma10)`
- [ERROR] 错误：`price_change.close > 0.03` - 禁止访问 Series 属性

### 6. 百分比表示
- [OK] 正确: `pct_chg > 0.03` (3%)
- [ERROR] 错误: `pct_chg > 3`

### 7. 工具返回值范围（**极其重要**）

**[WARN] 关键规则：必须根据工具的取值范围设置合理的阈值！**

**常见指标范围**:
- `outperform_rate`: **0-1**（例如 `>0.5` 表示超过一半时间跑赢）
- `beta`: 通常 0-2（<1 低波动，>1 高波动）
- `alpha`: 小数（0.01 表示1%超额收益）
- `correlation`: **-1 到 1**
- `rsi`: **0-100**
- `tracking_error`: 0-0.5
- `information_ratio`: -2 到 2

**[ERROR] 常见错误**:
- `(outperform_60 > 30)` - 不可能大于1
- `(rsi > 150)` - 不可能超过100

**📖 完整指南**: 详见 `.stock_asking/rules/tool-value-ranges.md`

### 8. 质量标准
- 候选数量根据实际筛选条件决定，不强制要求特定数量
- **重要：只返回符合筛选条件的股票，不要生成额外的"其他候选"或"备选股票"列表**
- 如果结果为空，检查：
  1. 表达式是否过于严格（如同时要求多个极端条件）
  2. **工具阈值是否在合理范围内**（参考第7条）
  3. 是否使用了不存在的字段或变量

**📖 详细标准**: 详见 `.stock_asking/skills/quality-assessment/SKILL.md`
...
### 10. 输出格式规范
- **只返回符合筛选条件的股票列表**
- **严禁**在回复中生成额外的“其他高波动候选”、“备选股票”等不符合主要筛选条件的列表
- **严禁**使用 Markdown 表格展示额外候选（如 `| 股票 | 代码 | 行业 | Beta | 跑赢率 |`）
- 如果用户要求宽松条件，应该在筛选逻辑中直接调整阈值，而不是生成两个列表
