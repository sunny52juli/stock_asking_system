...
**✅ 合理的表达式示例**:
- `(close > rolling_mean(close, 20))` - 收盘价高于20日均价
- `(vol > rolling_mean(vol, 5) * 1.5)` - 成交量放大1.5倍
- `(rsi14 > 30) & (rsi14 < 50)` - RSI在30-50之间
- `(ma5 > ma10) & (ma10 > ma20)` - 均线多头排列

**❌ 不合理的表达式示例（严禁使用）**:
- `(close > high_20)` - 收盘价不可能高于20日最高价（逻辑错误）
- `(vol < 0)` - 成交量不可能为负
- `(pe < -10)` - 市盈率通常为正值
- 同时要求多个极端条件导致结果为空

### 5. Tools 与 Expression 的对应关系（**关键规则**）

**⚠️ 重要：expression 中使用的所有变量必须在 tools 中定义！**

**✅ 正确示例**:
```json
{{
  "tools": [
    {{"tool": "rolling_mean", "params": {{"column": "vol", "window": 5}}, "var": "vol_ma5"}}
  ],
  "expression": "vol > vol_ma5 * 1.5"
}}
```
- tools 中定义了 `var: "vol_ma5"`
- expression 中使用了 `vol_ma5`
- ✅ 变量匹配
...
{{
  "tools": [
    {{"tool": "rolling_mean"}}  // ❌ 缺少 params 和 var
  ],
  "expression": "vol > vol_ma5 * 1.5"  // ❌ vol_ma5 未定义
}}
```

**规则总结**:
1. 每个工具必须包含 `tool`、`params`、`var` 三个字段
2. `var` 是变量的名称，会在 expression 中使用
3. expression 中使用的每个变量都必须在 tools 中有对应的 `var` 定义
4. 基础字段（如 `vol`, `close`, `high`）不需要在 tools 中定义，可以直接使用
