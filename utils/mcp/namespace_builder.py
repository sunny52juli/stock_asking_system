...
class NamespaceBuilder:
    ...
    def _infer_technical_indicator(data: pd.DataFrame, var_name: str) -> pd.Series | None:
        ...
        Returns:
            计算结果 Series 或 None
        """
        # {field}_avg_{window}d: N日平均
        avg_match = re.match(r'(open|high|low|close|vol|amount)_avg_(\d+)d', var_name, re.IGNORECASE)
        if avg_match:
            field = avg_match.group(1).lower()
            window = int(avg_match.group(2))
            if field in data.columns:
                return data[field].rolling(window=window).mean()
        
        # {field}_{window}d: N日值
        nd_match = re.match(r'(open|high|low|close|vol|amount)_(\d+)d', var_name, re.IGNORECASE)
        if nd_match:
            field = nd_match.group(1).lower()
            window = int(nd_match.group(2))
            if field in data.columns:
                return data[field].shift(window - 1)
