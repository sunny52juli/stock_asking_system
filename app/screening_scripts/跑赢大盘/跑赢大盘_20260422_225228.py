...
筛选说明:
    筛选低波动（beta<1）、跑赢大盘（alpha>0、跑赢比例>40%）的股票，使用相对排名确定置信度

工具步骤:
    1. beta_60: 通过工具 'beta' 计算，参数={'window': 60}
    2. alpha_60: 通过工具 'alpha' 计算，参数={'window': 60}
    3. outperform_60: 通过工具 'outperform_rate' 计算，参数={'window': 60}
    4. beta_rank: 通过工具 'rank_normalize' 计算，参数={'column': 'beta_60'}
    5. alpha_rank: 通过工具 'rank_normalize' 计算，参数={'column': 'alpha_60'}
    6. outperform_rank: 通过工具 'rank_normalize' 计算，参数={'column': 'outperform_60'}

筛选表达式: (beta_60 < 1.0) & (alpha_60 > 0) & (outperform_60 > 0.4)
置信度公式: rank_normalize(1 - beta_60) * 0.4 + rank_normalize(alpha_60) * 0.4 + rank_normalize(outperform_60) * 0.2

生成时间: 2026-04-22 22:52:28
"""
...
SCREENING_LOGIC = {
    ...
    "tools": [
        ...
        },
        {
            "tool": "rank_normalize",
            "params": {
                "column": "outperform_60"
            },
            "var": "outperform_rank"
        }
    ],
    "expression": "(beta_60 < 1.0) & (alpha_60 > 0) & (outperform_60 > 0.4)",
    "confidence_formula": "rank_normalize(1 - beta_60) * 0.4 + rank_normalize(alpha_60) * 0.4 + rank_normalize(outperform_60) * 0.2",
    "rationale": "筛选低波动（beta<1）、跑赢大盘（alpha>0、跑赢比例>40%）的股票，使用相对排名确定置信度"
}
# ==================== 筛选逻辑定义结束 ====================
