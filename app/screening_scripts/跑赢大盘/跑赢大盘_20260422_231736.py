...
筛选说明:
    筛选低波动(beta<1)、有超额收益(alpha>0)、跑赢比例超过45%、价格在20日均线之上的股票，使用排名加权计算置信度
...
工具步骤:
    ...
    5. beta_rank: 通过工具 'rank_normalize' 计算，参数={'column': 'beta_60'}
    6. alpha_rank: 通过工具 'rank_normalize' 计算，参数={'column': 'alpha_60'}

筛选表达式: (beta_60 < 1.0) & (alpha_60 > 0) & (outperform_60 > 0.45) & (close > ma20)
置信度公式: beta_rank * 0.4 + alpha_rank * 0.4 + rank_normalize(outperform_60) * 0.2

生成时间: 2026-04-22 23:17:36
"""
...
SCREENING_LOGIC = {
    ...
    "expression": "(beta_60 < 1.0) & (alpha_60 > 0) & (outperform_60 > 0.45) & (close > ma20)",
    "confidence_formula": "beta_rank * 0.4 + alpha_rank * 0.4 + rank_normalize(outperform_60) * 0.2",
    "rationale": "筛选低波动(beta<1)、有超额收益(alpha>0)、跑赢比例超过45%、价格在20日均线之上的股票，使用排名加权计算置信度"
}
# ==================== 筛选逻辑定义结束 ====================
